        ...
        for task_id, task in enumerate(batch_tasks):
            ...
            for i in range(0,num_inner_update_step):
                ...
                for inner_step, batch in enumerate(support_dataloader):
                    ...
                    loss += self.criterion(outputs, label_id) + 0.0001*sum([x.norm().pow(2) for x in adapter_model.parameters()]).sqrt()
                loss.backward()
            ...
            support_dataloader_ = DataLoader(support, sampler=None, batch_size=len(support), collate_fn=self.collate_pad_)
            support_batch = iter(support_dataloader_).__next__()
            q_input, q_label_id = query_batch
            q_outputs = adapter_model(predict(self.meta_model, q_input))
            q_loss = self.criterion(q_outputs, q_label_id)
            if training:
                hypergradient = stocbio(self.args, q_loss, self.meta_model, adapter_model, query_batch,  support_batch)
                if self.no_meta:
...
def stocbio(args, loss, meta_model, adapter_model, query_batch, support_batch):
    ...
    loss = F.cross_entropy(output, train_labels) + 0.0001*sum([x.norm().pow(2) for x in adapter_model.parameters()]).sqrt()
    G_gradient = []
    ...
    loss = F.cross_entropy(output, val_labels)+ 0.0001*sum([x.norm().pow(2) for x in adapter_model.parameters()]).sqrt()
