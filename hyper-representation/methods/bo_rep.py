...
class Learner(nn.Module):
    ...
    def forward(self, batch_tasks, training=True, task_batch_id=0, train_batch=None, epoch=0):
        ...
        num_inner_update_step = self.y_warm_start if (task_batch_id % self.args.interval)==0 else self.inner_update_step
        ...
        for taskid, task in enumerate(batch_tasks):
            ...
            for i in range(0, num_inner_update_step):
                # for inner_step, batch in enumerate(support_dataloader_y):
            ...
            if training:
                self.hyper_momentum, self.z_params = hypergradient(self.args, jacob_flat, self.hyper_momentum, self.z_params, inner_loss, \
                                                                   q_loss, self.meta_model, self.adapter_model, query_batch, support_batch)
...
def hypergradient(args, jacob_flat, hyper_momentum, z_params,  inner_loss, loss, meta_model, adapter_model, query_batch, support_batch):
